 ...
 * error 错误信息
 */
typedef void (^blueToothConnectDeviceCallback)(TFY_EasyPeripheral *perpheral,NSError *error,deviceConnectType deviceConnectType);
/**
 * 读取RSSI回调，次回掉之后会一次返回结果
 */
typedef void (^blueToothReadRSSICallback)(TFY_EasyPeripheral *peripheral , NSNumber *RSSI , NSError *error);

/**
 * 寻找设备中的服务回掉
 */
typedef void (^blueToothFindServiceCallback)(TFY_EasyPeripheral *peripheral , NSArray<TFY_EasyService *> *serviceArray , NSError * error);


@interface TFY_EasyPeripheral : NSObject
 ...
 * 设备当前的中心管理者
 */
@property (nonatomic, weak ,readonly) TFY_EasyCenterManager *centerManager;
 ...
 * 设备中所有的服务
 */
@property(nonatomic, strong) NSMutableArray<TFY_EasyService *> *serviceArray;
