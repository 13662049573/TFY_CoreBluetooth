 ...
 * 连接设备的时候蓝牙状态发生改变
 */
typedef void (^blueToothStateChanged)(TFY_EasyPeripheral *peripheral , bluetoothState state) ;

/**
 * 模糊搜索设备规则
 * 用户可以自定义，依据peripheral里面的名称，广播数据，RSSI来赛选需要的连接的设备
 */
typedef BOOL (^blueToothScanRule)(TFY_EasyPeripheral *peripheral);
 ...
 */
typedef void (^blueToothScanCallback)(TFY_EasyPeripheral *peripheral , NSError *error );
typedef void (^blueToothScanAsyncCallback)(TFY_EasyPeripheral *peripheral ,searchFlagType searchFlagType , NSError *error );
typedef void (^blueToothScanAllCallback)(NSArray<TFY_EasyPeripheral *> *deviceArray , NSError *error );
 ...
 *
 */
typedef void (^blueToothConnectCallback)(TFY_EasyPeripheral *peripheral , NSError *error );
...
@interface TFY_EasyBlueToothManager : NSObject


/**
 * 这个参数里面全都是放置的初始条件
 */
@property (nonatomic,strong)TFY_EasyManagerOptions *managerOptions ;
